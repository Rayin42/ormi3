...
//상품 카드 만드는 부분         $붙인건 DOM요소다!
//상품 이미지 주소, 가격, 상품 이름을 받으면 상품 카드를 만들어주는 함수
//외부 데이터 의존 없이 이 함수 기능만으로 완전하도록. 외부 데이터 부분은 따로 처리하도록 리팩토링
...
function main() {
    ...
    const products = fetch(BASE_URL + "mall").then((res) => {
        return res.json(); // json 배열요소
    }).then((json) => {
        json.forEach(data => {
            ...
            const onClick = async (e) => {           //then 대신 async await 사용 async로 묶으면 무조건 promise 반환
                // console.log(data.id+"번째 선택");
                detailContainer.innerHTML = "";
                const res = await fetch(BASE_URL + "mall/" + productId); // 이미 있는 데이터지만 연습삼아 한번 더 fetch로 받아오는 것! async await - promise를 기다렸다가 받겠다
